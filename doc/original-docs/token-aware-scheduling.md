...
|---|---|---|
| **L0 Pre-flight estimate** | `scheduler-estimate.sh`, `calibrate.sh` | Predict cost before fanning out; if confidence is LOW, probe one unit and re-estimate. |
| **L1 Live ceiling guard** | `ceiling-check.sh` (via `scheduler.sh gate`) | Halt the next wave when a live window reaches `100 − headroom`. Pure, deterministic, **fails closed**. |
...
| **L3 Off-peak scheduler** | `offpeak-window.sh`, `offpeak-budget.sh` | Run unattended in the quiet hours; reserve a morning allowance from the login-time answer. |
| **L4 Cheap resume** | `job-ledger.sh` | Resume does only what's LEFT — never re-derive context or re-fan the whole job. |
...
## The dispatcher's verdicts (`scheduler.sh`)

`scheduler.sh` owns **no arithmetic** — it sequences the tested helpers and names one verdict.
...
calls an orchestrator makes mid-turn. So `ratelimit-snapshot.sh` (a hook) writes the latest live
reading — the actual windows, percentages + reset epochs — to `ratelimit-snapshot.json`, stamped with
`captured_at`. `scheduler.sh gate` prefers a payload piped straight in (from a hook); only when none is
...
At job inception the scheduler asks **"what time will you log in tomorrow?"** and `offpeak-budget.sh`
...
**budget cap**, (3) it routes **every wave through `scheduler.sh gate`** (live ceiling, fail-closed),
...
## Needs empirical verification (probe before relying — `ratelimit-snapshot.sh`)

The snapshot bridge makes the guard robust regardless, but to know which mechanism is *primary*, run
`scheduler/verify-payload.sh` as a temporary hook and inspect the dumped payloads:

1. Does a `PreToolUse` (`Agent`/`Task`) hook receive `.rate_limits`? If yes, `preflight-fanout.sh` can
