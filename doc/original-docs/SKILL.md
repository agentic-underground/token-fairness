...
in the tested scripts under [`scheduler/`](../../scheduler/); your job is to **sequence them and obey
the verdicts**. Never compute a ceiling or estimate yourself — call the script. Protect the meter at
all costs; sacrifice parallelism first.

`S=${CLAUDE_PLUGIN_ROOT}/scheduler`
...
Pick or write a job profile (`scheduler/profiles/*.json`; project overrides in `.i2p/job-profiles/`).
A profile declares the unit, model, minimal tools/plugins/context (waste elimination), `max_parallel`,
and a token estimate. Then:

```bash
bash "$S/scheduler.sh" preflight --profile "$S/profiles/reviewer-fanout.json" --width 26
```
...
```bash
bash "$S/job-ledger.sh" init . "<job-id>" reviewer-fanout "unit1,unit2,…" <budget_total> <headroom>
bash "$S/job-ledger.sh" set-pointer . "<job-id>" cached_reviews_dir doc/cached-reviews
```
...
to a fresh snapshot by `ratelimit-snapshot.sh`), so the gate reads stdin-or-fresh-snapshot:

```bash
bash "$S/scheduler.sh" gate --headroom 15        # add --require-offpeak --now $(date +%s) for overnight
```

Act on the verdict — **do not override it**:

- **CONTINUE** → spawn the next wave (≤ `max_parallel`). After each unit completes, `job-ledger.sh
  mark-done`; record its real token cost via `calibrate.sh close <profile> <est> <actual>` so the next
  estimate sharpens.
- **HALT** → the live window hit the ceiling. `job-ledger.sh pause . <job-id> ceiling <pct> <reset>
...
```bash
bash "$S/offpeak-budget.sh" --now $(date +%s) --login <login-epoch> --reset <5h-reset-epoch> \
     --headroom 15 --morning-reserve 60
```

Use each window's `headroom` as the `--headroom` you pass to `scheduler.sh gate` for that window:
...
```bash
bash "$S/job-ledger.sh" remaining . "<job-id>"   # the worklist; everything else is already done
bash "$S/job-ledger.sh" resume . "<job-id>"
```
